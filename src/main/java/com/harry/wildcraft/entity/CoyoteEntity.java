...
public class CoyoteEntity extends Animal implements GeoEntity {
    ...
    private static final RawAnimation RANIM_DEATH  = RawAnimation.begin().thenPlay("animation.coyote.death");
    ...
    protected void registerGoals() {
        ...
        goalSelector.addGoal(4, new MeleeAttackGoal(this, 1.0, true) {
            @Override public boolean canUse() {
                ...
                if (level().getNearestPlayer(CoyoteEntity.this, STALK_DETECT_RANGE) != null) return false;
                return super.canUse();
            }
        ...
        targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, Player.class, false,
                e -> packState == PackState.ATTACKING && !((Player)e).isCreative() && !((Player)e).isSpectator()));
    ...
    public boolean hurt(DamageSource src, float dmg) {
        ...
        if (h) {
            ...
            if (src.getEntity() instanceof Player p && !p.isCreative() && !p.isSpectator()) {
    ...
    private static class CoyotePackPatrolGoal extends Goal {
        ...
        public CoyotePackPatrolGoal(CoyoteEntity coyote) { this.coyote = coyote; }
        @Override public boolean canUse() { return coyote.packState == PackState.IDLE && !coyote.level().isClientSide; }
        @Override public boolean canContinueToUse() { return coyote.packState == PackState.IDLE; }
        ...
        public void tick() {
            ...
            if (nearest != null && coyote.distanceTo(nearest) > PACK_LEASH_RANGE) {
                coyote.getNavigation().moveTo(nearest, 1.0);
                return;
            }
            if (mergeCheckTimer % 100 == 0) tryMergeWithNearbyPack();
            if (patrolTimer % 60 == 0) {
                ...
                if (packCenter != null) {
                    ...
                    coyote.getNavigation().moveTo(
                            packCenter.x + Math.cos(angle) * radius, packCenter.y,
                            packCenter.z + Math.sin(angle) * radius, 0.8);
                } else {
                    double angle = coyote.getRandom().nextDouble() * Math.PI * 2;
                    double dist = 5.0 + coyote.getRandom().nextDouble() * 10.0;
                    coyote.getNavigation().moveTo(
                            coyote.getX() + Math.cos(angle) * dist, coyote.getY(),
        ...
        private CoyoteEntity findNearestPackmate() {
            List<CoyoteEntity> pack = coyote.level().getEntitiesOfClass(CoyoteEntity.class,
                    coyote.getBoundingBox().inflate(PACK_COORDINATION_RANGE), c -> c != coyote && c.isAlive());
            return pack.stream().min((a, b) -> Double.compare(coyote.distanceTo(a), coyote.distanceTo(b))).orElse(null);
        }

        private Vec3 getPackCenter() {
            List<CoyoteEntity> pack = coyote.level().getEntitiesOfClass(CoyoteEntity.class,
                    coyote.getBoundingBox().inflate(PACK_LEASH_RANGE + 5), c -> c != coyote && c.isAlive());
            if (pack.isEmpty()) return null;
            double sumX = coyote.getX(), sumY = coyote.getY(), sumZ = coyote.getZ();
            for (CoyoteEntity c : pack) { sumX += c.getX(); sumY += c.getY(); sumZ += c.getZ(); }
            int count = pack.size() + 1;
            return new Vec3(sumX / count, sumY / count, sumZ / count);
        }

        private void tryMergeWithNearbyPack() {
            List<CoyoteEntity> distant = coyote.level().getEntitiesOfClass(CoyoteEntity.class,
                    coyote.getBoundingBox().inflate(PACK_MERGE_RANGE),
                    c -> c != coyote && c.isAlive() && coyote.distanceTo(c) > PACK_LEASH_RANGE);
            if (!distant.isEmpty()) {
                CoyoteEntity mergeTo = distant.stream()
                        .min((a, b) -> Double.compare(coyote.distanceTo(a), coyote.distanceTo(b))).orElse(null);
                if (mergeTo != null) coyote.getNavigation().moveTo(mergeTo, 0.9);
            }
        }
    }
    ...
    public void alertPackToStalk(Player target) {
        ...
        sl.getEntitiesOfClass(CoyoteEntity.class, getBoundingBox().inflate(PACK_COORDINATION_RANGE),
                c -> c != this && c.isAlive()).forEach(c -> {
    ...
    public void fireHowlThenAttack(Player target) {
        ...
        if (level() instanceof ServerLevel sl) {
            sl.getEntitiesOfClass(CoyoteEntity.class, getBoundingBox().inflate(30),
                    c -> c != this && c.isAlive()).forEach(c -> {
    ...
    public void setPackStateAll(PackState newState) {
        ...
        if (newState == PackState.IDLE && old != PackState.IDLE) { hasHowledForCombat = false; combatTimer = 0; }
        if (newState == PackState.STALKING) combatTimer = 0;
        if (level() instanceof ServerLevel sl) {
            sl.getEntitiesOfClass(CoyoteEntity.class, getBoundingBox().inflate(PACK_COORDINATION_RANGE),
                    c -> c != this && c.isAlive()).forEach(c -> {
                PackState cOld = c.packState;
                c.packState = newState;
                if (newState == PackState.IDLE && cOld != PackState.IDLE) { c.hasHowledForCombat = false; c.combatTimer = 0; }
                if (newState == PackState.STALKING) c.combatTimer = 0;
    ...
    public void tick() {
        ...
        if (hasTarget && packState == PackState.ATTACKING) combatTimer = COMBAT_TIMEOUT;
        else if (packState != PackState.ATTACKING) combatTimer = 0;
        else if (combatTimer > 0) {
            combatTimer--;
            if (combatTimer == 0 && !hasTarget) setPackStateAll(PackState.IDLE);
        }

        double speed = packState == PackState.ATTACKING ? 0.32 : packState == PackState.STALKING ? 0.18 : 0.22;
        if (getAttribute(Attributes.MOVEMENT_SPEED) != null)
            getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(speed);

        if (packState == PackState.ATTACKING && getTarget() instanceof Player p && distanceTo(p) > MAX_CHASE_RANGE) {
            setPackStateAll(PackState.IDLE);
            setTarget(null);
        }

        int newAnimState;
        if (fleeFromGazeTimer > 0) newAnimState = ANIM_SPRINT;
        else if (packState == PackState.ATTACKING || (combatTimer > 0 && hasTarget)) newAnimState = ANIM_SPRINT;
        else if (packState == PackState.STALKING) newAnimState = ANIM_STALK;
        else newAnimState = ANIM_WALK;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance diff,
        ...
        if (spawnType != MobSpawnType.SPAWNER && spawnType != MobSpawnType.SPAWN_EGG
            ...
            List<CoyoteEntity> existing = sl.getEntitiesOfClass(CoyoteEntity.class,
                    getBoundingBox().inflate(40), c -> c != this && c.isAlive());
            if (existing.size() >= 4) return spawnData;
            int groupSize = 1 + random.nextInt(2);
            for (int i = 0; i < groupSize; i++) {
                CoyoteEntity companion = new CoyoteEntity(com.harry.wildcraft.init.ModEntities.COYOTE.get(), sl);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar registrar) {
        registrar.add(new AnimationController<>(this, "main", 3, state -> {
            if (isDying()) return state.setAndContinue(RANIM_DEATH);
            if (isHowling()) {
