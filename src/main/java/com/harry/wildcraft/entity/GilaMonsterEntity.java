...
public class GilaMonsterEntity extends Animal implements GeoEntity {
    ...
    protected void registerGoals() {
        ...
        targetSelector.addGoal(2, new NearestAttackableTargetGoal<>(
                this, Player.class, true, e -> e.distanceTo(this) <= 1.0));
    }
    ...
    public boolean doHurtTarget(Entity target) {
        ...
        if (hit && target instanceof LivingEntity le)
            le.addEffect(new MobEffectInstance(MobEffects.POISON, 100, 0)); // 5 segundos
        return hit;
    }
    ...
    public boolean hurt(DamageSource src, float dmg) {
        ...
        if (hurt && src.getEntity() instanceof Player p) setTarget(p);
        return hurt;
    }
